...
def step_run_summary_command(context, filename):
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "anki_pleco_importer.cli", "summary", test_file],
            capture_output=True,
            text=True,
            cwd=os.getcwd(),
        )
...
def step_check_specific_card(context, card_num, expected_notetype, expected_chars):
    ...
    assert (
        card.notetype == expected_notetype
    ), f"Card {card_num} notetype: expected '{expected_notetype}', got '{card.notetype}'"
    assert (
        card.characters == expected_chars
    ), f"Card {card_num} characters: expected '{expected_chars}', got '{card.characters}'"
...
def step_check_summary_total(context, count):
    ...
    assert (
        f"Total cards: {count}" in context.summary_output
    ), f"Expected 'Total cards: {count}' in summary output: {context.summary_output}"
...
def step_check_summary_unique_characters(context, count):
    ...
    assert (
        f"Total unique characters: {count}" in context.summary_output
    ), f"Expected 'Total unique characters: {count}' in summary output: {context.summary_output}"
