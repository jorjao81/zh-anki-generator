...
def step_command_should_fail_invalid_api_key(context):
    ...
    assert context.exit_code != 0, f"Expected command to fail with invalid API key, but it succeeded. Stdout: {context.stdout}. Stderr: {context.stderr}"
        ...
        with patch('openai.OpenAI') as mock_openai:
            ...
            if command.startswith("anki-pleco-importer"):
                ...
                for i, part in enumerate(cmd_parts):
                    if part.endswith('.tsv') or part.endswith('.yaml') or part.endswith('.epub') or part.endswith('.txt'):
...
def step_command_should_succeed(context):
    ...
    assert context.exit_code == 0, f"Expected command to succeed, but it failed. Exit code: {context.exit_code}. Stdout: {context.stdout}. Stderr: {context.stderr}"
...
def step_output_should_show_formatted_meanings(context):
    ...
    assert '<span class="domain">mathematics</span>' in context.stdout, f"Expected domain markup in output. Got: {context.stdout}"
...
def step_cost_should_be_calculated_correctly(context, num_words):
    ...
    assert "Total AI cost" in context.stdout or "Cost:" in context.stdout, f"Expected cost information in output. Got: {context.stdout}"
    
    # Check that it mentions the correct number of words
    assert f"{num_words} word" in context.stdout, f"Expected mention of {num_words} words in output. Got: {context.stdout}"