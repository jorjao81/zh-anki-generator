...
class FeatureConfig(BaseModel):
    ...
    provider: str = "standard"  # "standard", "gpt", "gemini", "deepseek", or "qwen"
    ...
    def validate_provider(cls, v):
        valid_providers = ["standard", "gpt", "gemini", "deepseek", "qwen"]
...
class AIConfigLoader:
    ...
    def load_config(self) -> AIConfig:
        ...
        self._config = AIConfig(global_config=global_config, providers=providers, features=features)
    ...
    def _validate_feature_provider(
        self, feature_name: str, char_type: Optional[str], config, errors: List[str], warnings: List[str]
    ):
        ...
        if model:
            if provider == "gpt":
                ...
                if model not in valid_gpt_models:
                    warnings.append(
                        f"{feature_desc}: Unknown GPT model '{model}'. Known models: {', '.join(valid_gpt_models[:5])}..."
                    )
                ...
                if model not in valid_gemini_models:
                    warnings.append(
                        f"{feature_desc}: Unknown Gemini model '{model}'. Known models: {', '.join(valid_gemini_models)}"
                    )
            elif provider == "deepseek":
                valid_deepseek_models = ["deepseek-chat", "deepseek-reasoner"]
                if model not in valid_deepseek_models:
                    warnings.append(
                        f"{feature_desc}: Unknown DeepSeek model '{model}'. Known models: {', '.join(valid_deepseek_models)}"
                    )
            elif provider == "qwen":
                valid_qwen_models = [
                    "qwen-max",
                ...
                if model not in valid_qwen_models:
                    warnings.append(
                        f"{feature_desc}: Unknown Qwen model '{model}'. Known models: {', '.join(valid_qwen_models[:5])}..."
                    )
        else:
        ...
        if provider == "gpt":
            ...
            if not api_key:
                errors.append(
                    f"{feature_desc}: OpenAI API key not found. Set OPENAI_API_KEY environment variable or specify in providers.gpt.api_key"
                )
            else:
            ...
            if not api_key:
                errors.append(
                    f"{feature_desc}: Gemini API key not found. Set GEMINI_API_KEY environment variable or specify in providers.gemini.api_key"
                )
            else:
            ...
            if not api_key:
                errors.append(
                    f"{feature_desc}: DeepSeek API key not found. Set DEEPSEEK_API_KEY environment variable or specify in providers.deepseek.api_key"
                )
            else:
                self._test_api_key("deepseek", api_key, feature_desc, errors, warnings)
        elif provider == "qwen":
            api_key = self._get_api_key_for_provider("qwen")
            if not api_key:
                errors.append(
                    f"{feature_desc}: Qwen API key not found. Set QWEN_API_KEY environment variable or specify in providers.qwen.api_key"
                )
            else:
                self._test_api_key("qwen", api_key, feature_desc, errors, warnings)
