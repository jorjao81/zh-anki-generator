...
def _normalize_content(content: Union[str, List[str]]) -> str:
    """
    Normalize content to string format.
    
    Args:
        content: Either a string or list of strings
        
    Returns:
        String representation of the content
    """
    if isinstance(content, list):
        # Join list items with newlines for examples
        return "\n".join(str(item) for item in content)
    return str(content) if content is not None else ""


def _extract_content_safely(raw_content: str) -> str:
    """
    Safely extract formatted content from AI response.
    
    Handles cases where AI might return JSON instead of plain text.
    """
    if not raw_content:
        return ""
    ...
    try:
        ...
        if isinstance(parsed, list):
            ...
            # If it's a dict, try to extract a content field
            if "content" in parsed:
                return str(parsed["content"])
            elif "formatted_content" in parsed:
                return str(parsed["formatted_content"])
            elif "text" in parsed:
                return str(parsed["text"])
            else:
                # If no obvious content field, convert to string
                return str(parsed)
        else:
...
class BaseFieldFormatter(ABC):
    ...
    @abstractmethod
    def format_field(self, hanzi: str, original_content: Union[str, List[str]]) -> FieldFormattingResult:
...
class StandardFieldFormatter(BaseFieldFormatter):
    ...
    def format_field(self, hanzi: str, original_content: Union[str, List[str]]) -> FieldFormattingResult:
        ...
        normalized_content = _normalize_content(original_content)
        return FieldFormattingResult(formatted_content=normalized_content)
...
class AIFieldFormatter(BaseFieldFormatter):
    ...
    def format_field(self, hanzi: str, original_content: Union[str, List[str]]) -> FieldFormattingResult:
        ...
        # Normalize input content to string format
        normalized_content = _normalize_content(original_content)
        ...
        try:
            ...
            if provider in ["gpt", "deepseek"]:
                ...
                formatted_content = _extract_content_safely(raw_content)
